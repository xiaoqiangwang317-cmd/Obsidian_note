...
上一页我讲到，Agent Runtime 的核心不是一次模型调用，而是一个可持续执行、可观察、可回填上下文的闭环系统。那接下来这一页，我想说明我是怎么从开源项目 Nanobot 里观察这套 Runtime 思想的。

我选取 HKUDS/nanobot 作为参考对象，主要是因为它的代码规模比较轻量，但架构模块比较完整，能够比较清楚地看到一个 Agent Runtime 从启动、推理、能力调用到结果返回的主链路。

我把它的架构理解成四个部分。

第一部分是核心流转与大脑，也就是 Core Engine & Data Flow。它负责 Agent 的生命周期控制、会话状态维护、任务推进和上下文传递。这里对应的是 Runtime 最核心的问题：用户输入进来以后，系统如何判断当前任务状态，如何进入下一轮推理，如何把工具执行结果重新回填到上下文里。

第二部分是模型接入与能力扩展，也就是 LLM & Capabilities。这里主要负责模型 provider、本地 Tool、内置 Skill 以及能力描述。我的理解是，大模型在 Agent 系统里不是孤立存在的，它需要通过能力注册和能力描述，知道自己可以调用哪些工具、遵守哪些策略，以及如何把外部能力纳入推理过程。

第三部分是 I/O 接入与网关，也就是 Adapters & I/O。它负责把不同入口和外部系统接进来，比如 CLI、Web、MCP 或其他通信通道。这一层让我意识到，Agent 真正落地时不能只考虑模型本身，还要考虑它怎么接业务入口、怎么接外部服务、怎么把不同形式的输入统一到 Runtime 里。

第四部分是守护进程与系统运维，也就是 System & Infra。这里关注的是配置、错误处理、运行状态、权限、日志和稳定性。也就是说，一个 Agent 系统要进入工程应用，必须具备可运行、可排查、可恢复的基础能力，而不是只能在理想输入下跑通一次。

所以这一页我的重点不是说我完整复刻了 Nanobot，而是我从它的架构模块中提炼出一套实践路径：先做架构观察，理解 Runtime 需要哪些控制点；再做源码拆解，分析这些模块在代码里如何协作；然后用 Java 做自研验证，跑通状态机、上下文和工具路由；最后再把这些思想落到企业知识库智能问答系统里。

这里也能自然承接到后面的项目实现：Nanobot 的 Core Engine 思想，对应我项目里的多 Agent 状态机；Capabilities 对应 RAG、Prompt 和模型生成能力；Adapters 对应 Milvus、Redis、MySQL 等外部服务接入；System & Infra 对应 trace、审计日志、反馈和结果治理。

所以第三页想表达的是：我的项目不是直接从业务代码开始写，而是先参考 Nanobot 的架构模块理解 Agent Runtime，再把这些工程思想迁移到自己的 Java AI 应用系统中。
