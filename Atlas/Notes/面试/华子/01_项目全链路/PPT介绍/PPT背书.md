...
再往下是 Registry，也就是能力路由网关。它的作用是把不同能力统一注册和调度起来，比如本地 Tool、内置 Skill、远程 MCP。这样 Agent 不需要知道每个工具的具体实现细节，只需要根据任务需要选择合适的能力。这个设计让我感觉很重要，因为 AI 应用真正落地时，模型本身只是决策节点，背后还要接很多业务工具、知识库、数据库和外部服务。
